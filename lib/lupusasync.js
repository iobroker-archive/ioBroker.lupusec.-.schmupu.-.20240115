...
let urlDeviceGet = '/action/deviceGet';
...
function copyObject(obj) {
  let objcopy;
  // if (obj) objcopy = JSON.parse(JSONstringify(obj));
...
class SingletonHelper {
  ...
  async JSONparse(data) {
    data = delSonderzeichen(data);
    data = await JSON.parse(data);
    return data;
  }

  async requestGet(opt) {
    return await request.get(opt);
  }

  async requestPost(opt) {
...
class Log {
  ...
  debug(level, text) {
    ...
    if(this.level <= 0 || this.level === undefined) {
  ...
  info(text) {
    this.adapter.info(text);
  }

  error(text) {
    this.adapter.error(text);
  }
...
class Lupus {
  ...
  async JSONparse(data, resultname, resultreturn) {
    let result;
    resultreturn = (resultreturn === true);
    if (resultname && this.jsonparse[resultname] && this.jsonparse[resultname].data === data)
      return this.jsonparse[resultname].result;
    result = await JSON.parse(delSonderzeichen(data));
    if (resultname) {
      if (resultreturn === true) {
        this.jsonparse[resultname] = {
          data: data,
          result: result
        };
      } else {
  ...
  async requestGet(opt) {
    return await (await SingletonHelper.getInstance()).requestGet(opt);
  }

  async requestPost(opt) {
    return await (await SingletonHelper.getInstance()).requestPost(opt);
  }
  ...
  */
  ...
  async setPollSecOk() {
    if (this.errorWaitSec > 0) {
      this.log.info('Poll rythmus chnaged back after an Error from ' + this.errorWaitSec + ' to default values!');
      this.errorWaitSec = 0;
  ...
  async setPollSecErrorOld(message) {
    ...
    if (!alive) {
      this.log.error('Lupusec alarm system ' + this.hostname + ' is not reachable by ping.');
      return;
    }
    ...
    if ((now - this.errorTime) >= this.errorWaitSec) {
      ...
      this.log.error('Poll rythmus chnaged to ' + this.errorWaitSec + ' seconds because of error!');
      this.errorWaitSec += 15;
  ...
  async hostAliveError() {
    try {
      ...
      while (alive === false) {
        let waitsec = 15;
        this.log.error('Lupusec alarm system ' + this.hostname + ' is not reachable by ping. Wait ' + waitsec + ' seconds for next try!');
        await sleep(waitsec * 1000);
        alive = await pingAsync(this.hostname);
        if (alive === true) {
          this.log.info('Lupusec alarm system ' + this.hostname + ' is now reachable by ping again!');
          return true;
        }
      }
      return false;
    ...
    return new Promise(async (resolve) => {
      try {
        if (id && obj) {
          ...
          if(objold && objold.type == 'channel' && id.startsWith('devices') && !objold.common.statusStates) {
            objold.common.statusStates = {
              onlineId: 'cond_ok' 
            }
          ...
          if (obj && objold && obj.common && objold.common && obj.common.name !== objold.common.name || (obj.common.role !== objold.common.role)) {
            ...
            this.log.info('Changing properties of object ' + id);
            await this.adapter.extendObjectAsync(id, objold);
          }
        ...
        this.log.error('setObjectOrUpdate: ' + error);
        resolve(false);
      }
    });
  }
  ...
  async deleteDevices(ids) {
    if (!isEmpty(ids)) {
      ...
      for (let id in ids) {
        ...
        this.log.info('Deleting device : ' + id);
    ...
    try {
      ...
      for (let i in ids) {
        ...
        if (state && state.val) {
          ...
          if (parameter && parameter.hasOwnProperty('polltime')) {
            delete parameter.polltime;
          }
          if (!parameter || Object.keys(parameter).length == 0) {
            await this.adapter.delObjectAsync(id);
            this.log.debug(1,'Delete old object ' + id);
          } else {
            ...
            if (!propertiesObjAinObjB(parameter, obj.common)) {
              ...
              this.log.debug(1,'Changed parameters for object ' + id);
            }
          }
        }
      }
  ...
  async getToken(newtoken) {
    ...
    if (!this.token || !this.token.token || newtoken || Math.abs(getNow() - this.token.time) > this.tokentimeout) {
      ...
      this.log.debug(1,'Starting Token Request in getToken');
      this.log.debug(1,'Form of getToken (' + urlTokenGet + ') : ' + JSONstringify(opt));
      let body = await this.requestGet(opt);
      if (body) {
        body = await this.JSONparse(body);
        token = body.message;
        this.log.debug(1,'TokenGet : ' + JSONstringify(body));
      }
  ...
  async deviceGenericPost(url, form, resultname, resultreturn) {
    try {
      ...
      this.log.debug(1,'Form of deviceGenericPost (' + url + ') : ' + JSONstringify(opt));
      let body = await this.requestPost(opt);
      if (body) {
        body = await this.JSONparse(body, resultname, resultreturn);
        this.log.debug(1,'Result of deviceGenericPost (' + url + ') : ' + JSONstringify(body));
      }
      await this.setPollSecOk();
      if (body.result === 0) {
        switch (body.message) {
          case '{WEB_ERR_FORBIDDEN}':
            this.log.error('Authentication error in deviceGenericPost (' + url + ')');
            break;
          default:
            break;
        }
      ...
      if (error && error.error && error.error.code !== 'ESOCKETTIMEDOUT' && error.error.code !== 'ETIMEDOUT') {
        ...
        this.log.debug(1,'Error in deviceGenericPost (' + url + ') : ' + error);
      }
    }
  }

  async deviceGenericGet(url, form, resultname, resultreturn) {
    try {
      if (!await this.isPollingAlloed()) return;
      let opt = {
        url: this.getURL(url),
      ...
      if (form) opt.form = form;
      this.log.debug(1,'Form of deviceGenericGet (' + url + ') : ' + JSONstringify(opt));
      let body = await this.requestGet(opt);
      if (body) {
        body = await this.JSONparse(body, resultname, resultreturn);
        this.log.debug(1,'Result of deviceGenericGet (' + url + ') : ' + JSONstringify(body));
      }
      await this.setPollSecOk();
      if (body.result === 0) {
        switch (body.message) {
          case '{WEB_ERR_FORBIDDEN}':
            this.log.error('Authentication error in deviceGenericGet (' + url + ')');
            break;
          default:
            break;
        }
      ...
      if (error && error.error && error.error.code !== 'ESOCKETTIMEDOUT' && error.error.code !== 'ETIMEDOUT') {
        ...
        this.log.debug(1,'Warninng in deviceGenericGet (' + url + ') : ' + error);
      }
    }
  }
  ...
  async deviceSwitchDimmerPost(form) {
    ...
    let result = body && body.hasOwnProperty('result') ? body.result : undefined;
    return result;
  }
  ...
  async deviceEditPost(form) {
    ...
    let body = await this.deviceEditGetForPost(formget);
  ...
  async deviceEditGet(form) {
    let id = form && form.id ? urlDeviceEditGet + '_' + form.id : undefined;
    let body = await this.deviceGenericPost(urlDeviceEditGet, form, id);
    return body;
  }

  /**
   * DeviceEditGetForPost
   * @param {object} form - { id: 'zx122345' }
   */
  async deviceEditGetForPost(form) {
    let id = form && form.id ? urlDeviceEditGet + '_' + form.id + '_post' : undefined;
    let body = await this.deviceGenericPost(urlDeviceEditGet, form, id, true);
    return body;
  }
    ...
    if (object && Object.prototype.toString.call(object) === '[object Object]') {
      ...
      for (let statusname in newstates) {
        ...
        if (parameter) {
          ...
          // CPU Last veringern und bestimmte Objekte nicht bei jedem Durchlauf abfragen
          if (parameter.hasOwnProperty('polltime')) {
            let unixtime = Math.round(new Date().getTime());
            if (this.polltimes.hasOwnProperty(stateid) && (this.polltimes[stateid] + parameter.polltime) >= unixtime) {
              continue;
            }
            this.polltimes[stateid] = unixtime;
          }
  ...
  async deviceEditAllGet() {
    ...
    try {
      ...
      this.log.error('Fehler in funciton DeviceEditGet: ' + error);
    }
  }
  ...
  async deviceEditShutterGet(form) {
    let id = form && form.id ? urlDeviceEditShutterGet + '_' + form.id : undefined;
    let body = await this.deviceGenericPost(urlDeviceEditShutterGet, form, id);
    return body;
  }
  ...
  async deviceEditThermoGet(form) {
    let id = form && form.id ? urlDeviceEditThermoGet + '_' + form.id : undefined;
    let body = await this.deviceGenericPost(urlDeviceEditThermoGet, form, id);
    return body;
  }
  ...
  async deviceEditMeterGet(form) {
    let id = form && form.id ? urlDeviceEditMeterGet + '_' + form.id : undefined;
    let body = await this.deviceGenericPost(urlDeviceEditMeterGet, form, id);
    return body;
  }
   ...
   */
  async ipCamGet() {
    let body = await this.deviceGenericGet(urlIpcamGet, undefined, urlIpcamGet);
    let ipcamform = {};
  ...
  async systemGet(form) {
    let body = await this.deviceGenericGet(urlSystemGet, form, urlSystemGet, true);
    let sysform;
  ...
  async deviceListDevicePSSListSetStatus(object) {
    ...
    for (let devicename in object) {
      ...
      if (device.sid || device.id) {
        ...
        this.log.error('DeviceListDevicePSSListSetStatus : ID missing');
        continue;
      }
      await this.setObjectOrUpdate(channelid, {
        type: 'channel',
      ...
      for (let statusname in device) {
        ...
        if (statusname === 'name') {
          let tmpname = 'logmsg';
          let tmpid = channelid + '.' + tmpname;
          let tmpvalue = await this.getState(tmpid);
          newstates[tmpname] = {
            statusid: tmpid, // device.RFxxxxx.status
            statusname: tmpname, // status
            statusvalue: tmpvalue === undefined || tmpvalue === null ? '' : tmpvalue, // 1
            devicename: device.name, // Wohnzimmer Türsensor
            devicetype: device.type // 48
          };
        }
  ...
  async deviceListUPICGet() {
    let body = await this.deviceGenericGet(urlDeviceListUPICGet, undefined, urlDeviceListUPICGet);
    let upicrows;
  ...
  async logsGetPost() {
    ...
    body = await this.deviceGenericPost(urlLogsGet, form, urlLogsGet);
  ...
  async deviceGet() {
    let body = await this.deviceGenericGet(urlDeviceGet, undefined, urlDeviceGet);
    let senrows;
  ...
  async devicePSSListGet() {
    let body = await this.deviceGenericGet(urlDevicePSSListGet, undefined, urlDevicePSSListGet);
    let pssrows;
  ...
  async deviceListGet() {
    let body = await this.deviceGenericGet(urlDeviceListGet, undefined, urlDeviceListGet);
    let senrows;
    if (body && body.senrows && !isEmpty(body.senrows)) {
      senrows = body.senrows;
      await this.deviceListDevicePSSListSetStatus(senrows);
  ...
  async panelCondGet() {
    ...
    let devicename = 'Status Alarmanlage';
    let objectnew = {};
    let object = await this.deviceGenericGet(urlPanelCondGet, undefined, urlPanelCondGet);
    let promises = [];
